...
export const addMember = asyncHandler(async (req: Request, res: Response) => {
    const { id } = req.params;
    const member = await chatDb.getUserByEmail(req.body.memberEmail)
    if (!member) {
...
export const getChatofGroup = asyncHandler(async (req: Request, res: Response) => {
    const { id } = req.params;
    const result = await chatDb.getgroupMsg(id)
    res.send(createResponse(result, "Success"));
    return
})
...
export const getMembers = asyncHandler(async (req: Request, res: Response)=>{
    ...
    console.log('id', id)
    const result = await chatDb.getgroupMembers(id);
    res.send(createResponse(result, "Success"));
    return
})